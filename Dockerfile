...
RUN apt-get update && apt-get install -yq gnupg2 && apt-get clean
...
RUN apt-get update && apt-get install -yq --no-install-recommends \
  salt-minion vim ssh less net-tools procps lsb-release ifupdown && apt-get clean

RUN apt-get purge -yq gnupg2
